...
public:
	LogWindow(const std::string& title) : title(title) {
		clear();
	}
	...
	void draw() {
		...
		if (ImGui::BeginChild("scrolling", ImVec2(0, 0), false, ImGuiWindowFlags_HorizontalScrollbar)) {
			...
			if (filter.IsActive()) {
				// Don't use the clipper when Filter is enabled because we don't have random access to the result of the filter
				for (int lineNum = 0; lineNum < lineOffsets.size(); lineNum++) {
					const char* lineStart = bufBegin + lineOffsets[lineNum];
					const char* lineEnd = (lineNum + 1 < lineOffsets.size()) ? (bufBegin + lineOffsets[lineNum + 1] - 1) : bufEnd;
...
// static ll::io::StdoutRedirector stdoutRedirector(ll::io::StdoutRedirector::StandardOutput);
...
void updateImGui() {
	...
	// logWindow.addLog(stdoutRedirector.read());
	...
	if (showPerformance) {
		...
		if (ImGui::Begin("Performance", &showPerformance)) {
			...
			ImGui::Text("Renderer: %s", ImGuiHooks::getRendererType().c_str());
			ImGui::Text("Renderer Name: %s", ImGuiHooks::getGPUName().c_str());
