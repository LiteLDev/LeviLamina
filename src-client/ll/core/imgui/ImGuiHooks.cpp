...
static void replaceVtable(void* vptr, size_t index, void** outOldFunc, void* newFunc) {
	...
	ll::memory::modify(&ptr[index], sizeof(void*), [=]() {
		ptr[index] = newFunc;
	});
}
...
namespace ImGuiHooks {
	...
	namespace D3D12 {
		...
		HRESULT STDMETHODCALLTYPE IDXGISwapChain_ResizeBuffers_Hook(IDXGISwapChain* This, UINT BufferCount, UINT Width, UINT Height, DXGI_FORMAT NewFormat, UINT SwapChainFlags) {
			releaseRT();
			HRESULT hResult = IDXGISwapChain_ResizeBuffers_Original(This, BufferCount, Width, Height, NewFormat, SwapChainFlags);
			createRT(This);
			return hResult;
		}
	}
	...
	HRESULT STDMETHODCALLTYPE IDXGIFactory2_CreateSwapChainForCoreWindow_Hook(IDXGIFactory2* This, IUnknown* pDevice, IUnknown* pWindow, const DXGI_SWAP_CHAIN_DESC1* pDesc, IDXGIOutput* pRestrictToOutput, IDXGISwapChain1** ppSwapChain) {
		...
		HRESULT hResult = IDXGIFactory2_CreateSwapChainForCoreWindow_Original(This, pDevice, pWindow, pDesc, pRestrictToOutput, ppSwapChain);
		if (SUCCEEDED(hResult)) {
			...
			if (SUCCEEDED(pDevice->QueryInterface(&d3d12CommandQueue))) {
				//Direct3D 12
				ImGuiHooks::D3D12::commandQueue = (ID3D12CommandQueue*)pDevice;
				if (!ImGuiHooks::D3D12::IDXGISwapChain_Present_Original) {
					replaceVtable(*(void**)swapChain, 8, (void**)&ImGuiHooks::D3D12::IDXGISwapChain_Present_Original, ImGuiHooks::D3D12::IDXGISwapChain_Present_Hook);
				}
				if (!ImGuiHooks::D3D12::IDXGISwapChain_ResizeBuffers_Original) {
					replaceVtable(*(void**)swapChain, 13, (void**)&ImGuiHooks::D3D12::IDXGISwapChain_ResizeBuffers_Original, ImGuiHooks::D3D12::IDXGISwapChain_ResizeBuffers_Hook);
				}
				...
				//Direct3D 11
				ImGuiHooks::D3D11::device = (ID3D11Device*)pDevice;
				if (!ImGuiHooks::D3D11::IDXGISwapChain_Present_Original)
					replaceVtable(*(void**)swapChain, 8, (void**)&ImGuiHooks::D3D11::IDXGISwapChain_Present_Original, ImGuiHooks::D3D11::IDXGISwapChain_Present_Hook);
				if (!ImGuiHooks::D3D11::IDXGISwapChain_ResizeBuffers_Original)
					replaceVtable(*(void**)swapChain, 13, (void**)&ImGuiHooks::D3D11::IDXGISwapChain_ResizeBuffers_Original, ImGuiHooks::D3D11::IDXGISwapChain_ResizeBuffers_Hook);
				...
				imguiInitialized = ImGuiHooks::D3D11::initializeImguiBackend(swapChain);
				// if (!imguiInitialized) {
	...
	HRESULT createDXGIFactory1Hook(REFIID riid, void** ppFactory) {
		...
		if (IsEqualIID(IID_IDXGIFactory2, riid) && SUCCEEDED(hResult)) {
			...
			if (!IDXGIFactory2_CreateSwapChainForCoreWindow_Original) {
				replaceVtable(*(void**)factory2, 16, (void**)&IDXGIFactory2_CreateSwapChainForCoreWindow_Original, IDXGIFactory2_CreateSwapChainForCoreWindow_Hook);
			}
			ImGuiHooks::factory = factory2;
		}
		return hResult;
	}

	void init() {
		ll::memory::hook(CreateDXGIFactory1, createDXGIFactory1Hook, (ll::memory::FuncPtr*)&createDXGIFactory1Original, ll::memory::HookPriority::Normal);
	}
}
