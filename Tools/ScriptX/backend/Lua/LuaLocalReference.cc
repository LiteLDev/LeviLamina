...
bool judgeIsArray(int index) {
  ...
  while (lua_next(lua, index)) {
    // Copy current key and judge its type
    lua_pushvalue(lua, -2);
    if (!lua_isnumber(lua, -1) || lua_tonumber(lua, -1) != ++currentArrIndex) {
      lua_pop(lua, 3);
      isArray = false;
    }
    lua_pop(lua, 2);
...
ValueKind Local<Value>::getKind() const {
  ...
  if (type <= 0) {
    ...
    // // lua don't have array type, they are all tables
...
bool Local<Value>::isArray() const {
  if (val_ == 0 || lua_type(lua_backend::currentLua(), val_) != LUA_TTABLE) return false;
