...
#define InitFormat(type) mValueFormats[magic_enum::enum_integer(type)] = {DefaultPrefix<type>, DefaultSuffix<type>};
    ForEachTagType(InitFormat) mExpandInList[magic_enum::enum_integer(Tag::Type::End)] = false;
...
inline bool PrettySnbtFormat::setValueColor(mce::Color const& color) {
    if constexpr (type == Tag::Type::String)
        mValueFormats[magic_enum::enum_integer(type)] = {
            getColorCode(color) + DefaultPrefix<type>, DefaultSuffix<type> + getResetColorCode()};
    else if constexpr (type >= Tag::Type::ByteArray)
        mValueFormats[magic_enum::enum_integer(type)] = {
            getColorCode(color) + DefaultPrefix<type> + getResetColorCode(),
            getColorCode(color) + DefaultSuffix<type> + getResetColorCode()};
    else
        mValueFormats[magic_enum::enum_integer(type)] = {
            getColorCode(color) + DefaultPrefix<type>, getItalicCode() + DefaultSuffix<type> + getResetColorCode()};
    return true;
}
...
#define DllExport_SetValueColor(type) template LLAPI bool PrettySnbtFormat::setValueColor<type>(mce::Color const& color)
...
#define DllExport_SetExpand(type) template LLAPI bool PrettySnbtFormat::setExpand<type>(bool expand)
...
inline std::string PrettySnbtFormat::getResetColorCode() const { return mForPlayer ? "§r" : "\x1b[0m"; }

inline std::string PrettySnbtFormat::getItalicCode() const { return mForPlayer ? "§o" : "\x1b[3m"; }
...
inline bool PrettySnbtFormat::isPlayerFormat() const { return mForPlayer; }
...
inline bool PrettySnbtFormat::setColonColor(mce::Color const& color) { return setStringColor(mColon, color); }

inline bool PrettySnbtFormat::setSeparatorColor(mce::Color const& color) { return setStringColor(mSeparator, color); }
