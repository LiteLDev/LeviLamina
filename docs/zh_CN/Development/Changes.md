...
> 由于自1.0.0版本开始，API设计严格遵循向下兼容的原则，因此几乎不再出现导致API不兼容的重大更改，无需再不断维护此页面  
> 如果不出意外的话，此页面将不再更新，极少数出现的特殊情况将在版本发布时一并说明

这里列出了LiteLoader ScriptEngine对 **API** 进行的一些可能会造成 **不向下兼容** 的重大更改。  
