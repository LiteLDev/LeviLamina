...
public:
    ...
    inline unsigned __int64 registerLoop(std::string const& a0, class std::function<void (struct LoopingSoundState&)> a1, float a2, float a3){
        unsigned __int64 (NullSoundPlayer::*rv)(std::string const&, class std::function<void (struct LoopingSoundState&)>, float, float);
        *((void**)&rv) = dlsym("?registerLoop@NullSoundPlayer@@UEAA_KAEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@V?$function@$$A6AXAEAULoopingSoundState@@@Z@3@MM@Z");
        return (this->*rv)(std::forward<std::string const&>(a0), std::forward<class std::function<void (struct LoopingSoundState&)>>(a1), std::forward<float>(a2), std::forward<float>(a3));
    }
