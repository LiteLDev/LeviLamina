// This Header is auto generated by BDSLiteLoader Toolchain
#pragma once
#include "../Global.h"
#define EXTRA_INCLUDE_PART_SCRIPTBINDERPUREECSTEMPLATE
#include "Extra/ScriptBinderPureEcsTemplateAPI.hpp"
#undef EXTRA_INCLUDE_PART_SCRIPTBINDERPUREECSTEMPLATE
class ScriptBinderPureEcsTemplate {
#include "Extra/ScriptBinderPureEcsTemplateAPI.hpp"
public:
    /*0*/ virtual ~ScriptBinderPureEcsTemplate();
    /*1*/ virtual std::string const& getTemplateIdentifier() const;
    /*2*/ virtual void applyTemplate(class ScriptObjectBinder&) const;
    /*
    inline void applyTemplate(class ScriptObjectBinder& a0) const{
        void (ScriptBinderPureEcsTemplate::*rv)(class ScriptObjectBinder&) const;
        *((void**)&rv) = dlsym("?applyTemplate@ScriptBinderPureEcsTemplate@@UEBAXAEAVScriptObjectBinder@@@Z");
        return (this->*rv)(std::forward<class ScriptObjectBinder&>(a0));
    }
    inline std::string const& getTemplateIdentifier() const{
        std::string const& (ScriptBinderPureEcsTemplate::*rv)() const;
        *((void**)&rv) = dlsym("?getTemplateIdentifier@ScriptBinderPureEcsTemplate@@UEBAAEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@XZ");
        return (this->*rv)();
    }
    */
    MCAPI static std::unique_ptr<class ScriptObjectBinder> build(class entt::basic_registry<enum entt::entity>&, enum entt::entity);

protected:

private:
};