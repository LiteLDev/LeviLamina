// This Header is auto generated by BDSLiteLoader Toolchain
#pragma once
#define AUTO_GENERATED
#include "../Global.h"
#include "Actor.hpp"
#include "Minecart.hpp"

#define BEFORE_EXTRA
// Include Headers or Declare Types Here

#undef BEFORE_EXTRA

class MinecartChest : public Minecart {

#define AFTER_EXTRA
// Add Member There

#undef AFTER_EXTRA

#ifndef DISABLE_CONSTRUCTOR_PREVENTION_MINECARTCHEST
public:
    class MinecartChest& operator=(class MinecartChest const&) = delete;
    MinecartChest(class MinecartChest const&) = delete;
    MinecartChest() = delete;
#endif

public:
    /*0*/ virtual ~MinecartChest();
    /*1*/ virtual void __unk_vfn_0();
    /*2*/ virtual void __unk_vfn_1();
    /*3*/ virtual float getInterpolatedHeadRot(float) const;
    /*4*/ virtual class Vec3 getInterpolatedRidingOffset(float) const;
    /*5*/ virtual void __unk_vfn_2();
    /*6*/ virtual void lerpMotion(class Vec3 const&);
    /*7*/ virtual float getRidingHeight();
    /*8*/ virtual void __unk_vfn_3();
    /*9*/ virtual void __unk_vfn_4();
    /*10*/ virtual float getCameraOffset() const;
    /*11*/ virtual float getShadowHeightOffs();
    /*12*/ virtual float getShadowRadius() const;
    /*13*/ virtual void __unk_vfn_5();
    /*14*/ virtual void __unk_vfn_6();
    /*15*/ virtual void playerTouch(class Player&);
    /*16*/ virtual void __unk_vfn_7();
    /*17*/ virtual void __unk_vfn_8();
    /*18*/ virtual void __unk_vfn_9();
    /*19*/ virtual void setSleeping(bool);
    /*20*/ virtual void __unk_vfn_10();
    /*21*/ virtual void __unk_vfn_11();
    /*22*/ virtual void __unk_vfn_12();
    /*23*/ virtual void __unk_vfn_13();
    /*24*/ virtual void __unk_vfn_14();
    /*25*/ virtual void __unk_vfn_15();
    /*26*/ virtual void __unk_vfn_16();
    /*27*/ virtual void __unk_vfn_17();
    /*28*/ virtual class Actor* findAttackTarget();
    /*29*/ virtual void __unk_vfn_18();
    /*30*/ virtual void adjustDamageAmount(int&) const;
    /*31*/ virtual void onTame();
    /*32*/ virtual void onFailedTame();
    /*33*/ virtual void __unk_vfn_19();
    /*34*/ virtual void vehicleLanded(class Vec3 const&, class Vec3 const&);
    /*35*/ virtual bool isInvulnerableTo(class ActorDamageSource const&) const;
    /*36*/ virtual void animateHurt();
    /*37*/ virtual float getPickRadius();
    /*38*/ virtual void awardKillScore(class Actor&, int);
    /*39*/ virtual enum ArmorMaterialType getArmorMaterialTypeInSlot(enum ArmorSlot) const;
    /*40*/ virtual enum ArmorTextureType getArmorMaterialTextureTypeInSlot(enum ArmorSlot) const;
    /*41*/ virtual float getArmorColorInSlot(enum ArmorSlot, int) const;
    /*42*/ virtual void setEquippedSlot(enum EquipmentSlot, class ItemStack const&);
    /*43*/ virtual class HashedString const& queryEntityRenderer() const;
    /*44*/ virtual struct ActorUniqueID getSourceUniqueID() const;
    /*45*/ virtual void __unk_vfn_20();
    /*46*/ virtual int getPortalWaitTime() const;
    /*47*/ virtual void __unk_vfn_21();
    /*48*/ virtual void __unk_vfn_22();
    /*49*/ virtual struct ActorUniqueID getControllingPlayer() const;
    /*50*/ virtual void __unk_vfn_23();
    /*51*/ virtual void __unk_vfn_24();
    /*52*/ virtual void __unk_vfn_25();
    /*53*/ virtual void __unk_vfn_26();
    /*54*/ virtual void __unk_vfn_27();
    /*55*/ virtual void buildDebugInfo(std::string&) const;
    /*56*/ virtual int getDeathTime() const;
    /*57*/ virtual void swing();
    /*58*/ virtual void __unk_vfn_28();
    /*59*/ virtual void __unk_vfn_29();
    /*60*/ virtual float getYHeadRot() const;
    /*61*/ virtual void __unk_vfn_30();
    /*62*/ virtual void __unk_vfn_31();
    /*63*/ virtual void __unk_vfn_32();
    /*64*/ virtual void __unk_vfn_33();
    /*65*/ virtual void setAuxValue(int);
    /*66*/ virtual void stopSpinAttack();
    /*67*/ virtual void __unk_vfn_34();
    /*68*/ virtual void __unk_vfn_35();
    /*69*/ virtual void updateEntitySpecificMolangVariables(class RenderParams&);
    /*70*/ virtual void __unk_vfn_36();
    /*71*/ virtual void __unk_vfn_37();
    /*72*/ virtual void __unk_vfn_38();
    /*73*/ virtual void destroy(class ActorDamageSource const&, bool);
    /*74*/ virtual enum MinecartType getType();
    /*75*/ virtual class Block const* getDefaultDisplayBlock() const;
    /*76*/ virtual void __unk_vfn_39();
    /*77*/ virtual int getDefaultDisplayOffset() const;
    /*78*/ virtual void applyNaturalSlowdown(class BlockSource&);
    /*
    inline void applyNaturalSlowdown(class BlockSource& a0){
        void (MinecartChest::*rv)(class BlockSource&);
        *((void**)&rv) = dlsym("?applyNaturalSlowdown@MinecartChest@@UEAAXAEAVBlockSource@@@Z");
        return (this->*rv)(std::forward<class BlockSource&>(a0));
    }
    */
    MCAPI MinecartChest(class ActorDefinitionGroup*, struct ActorDefinitionIdentifier const&, class EntityContext&);

protected:

private:
    MCAPI static int const ITEMS_SIZE;

};