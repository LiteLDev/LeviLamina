...
class Result {
  public:
    Result(const std::shared_ptr<Response> &res, Error err) : res_(res), err_(err) {}
...
inline bool wait_until_socket_is_ready(socket_t sock, time_t sec, time_t usec) {
    ...
    if (ret > 0 && (FD_ISSET(sock, &fdsr) || FD_ISSET(sock, &fdsw))) {
        int error     = 0;
        socklen_t len = sizeof(error);
        return getsockopt(sock, SOL_SOCKET, SO_ERROR, reinterpret_cast<char *>(&error), &len) >=
                   0 &&
               !error;
    }
    return false;
...
class brotli_compressor : public compressor {
  public:
    brotli_compressor() { state_ = BrotliEncoderCreateInstance(nullptr, nullptr, nullptr); }
...
inline std::pair<std::string, std::string> make_digest_authentication_header(
    ...
    {
        auto H =
            algo == "SHA-256" ? detail::SHA_256 : algo == "SHA-512" ? detail::SHA_512 : detail::MD5;
    ...
    auto field = "Digest username=\"" + username + "\", realm=\"" + auth.at("realm") +
                 ...
                 "\", algorithm=" + algo + ", qop=" + qop + ", nc=\"" + nc + "\", cnonce=\"" +
    ...
    if (res.body.empty()) {
        if (res.content_length_ > 0) {
            ...
            if (req.ranges.empty()) {
                ...
                auto content_range =
                    detail::make_content_range_header_field(offset, length, res.content_length_);
        ...
        if (req.ranges.empty()) {
            ...
            auto length  = offsets.second;
            auto content_range =
                detail::make_content_range_header_field(offset, length, res.body.size());
            res.set_header("Content-Range", content_range);
...
inline bool Server::write_content_with_provider(Stream &strm,
    ...
    if (res.content_length_ > 0) {
        if (req.ranges.empty()) {
            if (detail::write_content(strm, res.content_provider_, 0, res.content_length_,
                                      is_shutting_down) < 0) {
                return false;
            }
        } else if (req.ranges.size() == 1) {
        ...
        if (res.is_chunked_content_provider) {
            ...
            if (detail::write_content_chunked(strm, res.content_provider_, is_shutting_down,
                                              *compressor) < 0) {
                return false;
            }
        } else {
    ...
    if (detail::expect_content(req)) {
        // Content reader handler
        {
            ContentReader reader(
                [&](ContentReceiver receiver) {
                    return read_content_with_content_receiver(strm, req, res, receiver, nullptr,
                                                              nullptr);
                },
                [&](MultipartContentHeader header, ContentReceiver receiver) {
                    return read_content_with_content_receiver(strm, req, res, nullptr, header,
                                                              receiver);
                });

            if (req.method == "POST") {
                if (dispatch_request_for_content_reader(req, res, reader,
...
inline bool Server::process_and_close_socket(socket_t sock) {
    auto ret = detail::process_server_socket(
        sock, keep_alive_max_count_, keep_alive_timeout_sec_, read_timeout_sec_, read_timeout_usec_,
...
inline socket_t ClientImpl::create_client_socket() const {
    if (!proxy_host_.empty() && proxy_port_ != -1) {
        return detail::create_client_socket(proxy_host_.c_str(), proxy_port_, tcp_nodelay_,
                                            socket_options_, connection_timeout_sec_,
                                            connection_timeout_usec_, interface_, error_);
    }
    return detail::create_client_socket(host_.c_str(), port_, tcp_nodelay_, socket_options_,
...
inline void ClientImpl::close_socket(Socket &socket, bool /*process_socket_ret*/) {
...
inline bool ClientImpl::send(const Request &req, Response &res) {
    ...
    auto ret = process_socket(
        socket_, [&](Stream &strm) { return handle_request(strm, req, res, close_connection); });
...
inline bool ClientImpl::write_request(Stream &strm, const Request &req, bool close_connection) {
    detail::BufferStream bstrm;
    ...
    if (!proxy_basic_auth_username_.empty() && !proxy_basic_auth_password_.empty()) {
        headers.insert(make_basic_authentication_header(proxy_basic_auth_username_,
                                                        proxy_basic_auth_password_, true));
    }

    if (!bearer_token_auth_token_.empty()) {
        headers.insert(make_bearer_token_authentication_header(bearer_token_auth_token_, false));
    }
...
inline bool ClientImpl::process_socket(Socket &socket, std::function<bool(Stream &strm)> callback) {
...
inline Result ClientImpl::Get2(std::string_view path, ClientOptions options) {
    ...
    return Get(path2.c_str(), std::move(options.headers), std::move(options.response_handler),
...
inline Result ClientImpl::Post(const char *path,
                               ...
                               const std::string &body,
                               const char *content_type) {
    auto ret = send_with_content_provider("POST", path, headers, body, 0, nullptr, content_type);
...
inline Result ClientImpl::Post(const char *path,
                               ...
                               ContentProvider content_provider,
                               const char *content_type) {
    auto ret = send_with_content_provider("POST", path, headers, std::string(), content_length,
...
inline Result ClientImpl::Put(const char *path,
                              ...
                              const std::string &body,
                              const char *content_type) {
    auto ret = send_with_content_provider("PUT", path, headers, body, 0, nullptr, content_type);
    return Result{ret, get_last_error()};
}
...
inline Result ClientImpl::Put(const char *path,
                              ...
                              ContentProvider content_provider,
                              const char *content_type) {
    auto ret = send_with_content_provider("PUT", path, headers, std::string(), content_length,
                                          content_provider, content_type);
    return Result{ret, get_last_error()};
}
...
inline Result ClientImpl::Patch(const char *path,
                                ...
                                const std::string &body,
                                const char *content_type) {
    auto ret = send_with_content_provider("PATCH", path, headers, body, 0, nullptr, content_type);
    return Result{ret, get_last_error()};
}
...
inline Result ClientImpl::Patch(const char *path,
                                ...
                                ContentProvider content_provider,
                                const char *content_type) {
    auto ret = send_with_content_provider("PATCH", path, headers, std::string(), content_length,
                                          content_provider, content_type);
    return Result{ret, get_last_error()};
}
...
inline bool SSLServer::process_and_close_socket(socket_t sock) {
    auto ssl =
        detail::ssl_new(sock, ctx_, ctx_mutex_, SSL_accept, [](SSL * /*ssl*/) { return true; });

    if (ssl) {
        auto ret = detail::process_server_socket_ssl(
            ssl, sock, keep_alive_max_count_, keep_alive_timeout_sec_, read_timeout_sec_,
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
            cli_ =
                std::make_shared<SSLClient>(host.c_str(), port, client_cert_path, client_key_path);
            is_ssl_ = is_ssl;
#endif
        } else {
            cli_ =
                std::make_shared<ClientImpl>(host.c_str(), port, client_cert_path, client_key_path);
        }
    } else {
        cli_ =
            std::make_shared<ClientImpl>(scheme_host_port, 80, client_cert_path, client_key_path);
    }
}
...
inline Client::Client(const std::string &host,
                      ...
                      const std::string &client_cert_path,
                      const std::string &client_key_path)
    : cli_(std::make_shared<ClientImpl>(host, port, client_cert_path, client_key_path)) {}
