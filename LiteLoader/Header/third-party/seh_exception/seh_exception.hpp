...
__declspec(dllimport) unsigned long __stdcall FormatMessageA(unsigned long dwFlags, const char* lpSource, unsigned long dwMessageId, unsigned long dwLanguageId, char* lpBuffer, unsigned long nSize, char* Arguments);
__declspec(dllimport) void* __stdcall LocalFree(void* hMem);
}
namespace WinFormatMessageFlags {
...
public:
    ...
    const char* what() const noexcept {
        ...
        unsigned long messageLength = FormatMessageA(
            WinFormatMessageFlags::ALLOCATE_BUFFER | WinFormatMessageFlags::FROM_SYSTEM | WinFormatMessageFlags::FROM_HMODULE,
            nullptr,
            RtlNtStatusToDosError(_expCode),
            0,
            (char*)&_expMsg,
            0,
            nullptr);
    ...
    */
    static void __cdecl TranslateSEHtoCE(unsigned int ExpCode, SEH_EXP_INFO_POINTER ExpInfo) {
