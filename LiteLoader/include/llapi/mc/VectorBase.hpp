...
template <class T>
constexpr std::size_t hash_combine(T const& v, std::size_t& seed) {
...
#define FAKE_CRTP(T, _t_type_, N)                                                                     \
    ...
    [[nodiscard]] inline std::size_t hash() const {                                                   \
        std::size_t res = 0;                                                                          \
        unroll<N>([&](size_t iter) {                                                                  \
            if constexpr (std::is_integral_v<_t_type_>) {                                             \
                hash_combine(operator[](iter), res);                                                  \
            } else {                                                                                  \
                hash_combine(std::hash<_t_type_>()(operator[](iter)), res);                           \
            }                                                                                         \
        });                                                                                           \
        return res;                                                                                   \
    }                                                                                                 \
