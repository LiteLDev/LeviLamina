/**
 * @file  ScriptCommandMessageEvent.hpp
 *
 */
#pragma once
#define AUTO_GENERATED
#include "llapi/Global.h"

#define BEFORE_EXTRA

#undef BEFORE_EXTRA


struct ScriptCommandMessageEvent {

#define AFTER_EXTRA

#undef AFTER_EXTRA
#ifndef DISABLE_CONSTRUCTOR_PREVENTION_SCRIPTCOMMANDMESSAGEEVENT
public:
    struct ScriptCommandMessageEvent& operator=(struct ScriptCommandMessageEvent const &) = delete;
    ScriptCommandMessageEvent(struct ScriptCommandMessageEvent const &) = delete;
    ScriptCommandMessageEvent() = delete;
#endif

public:
    /**
     * @symbol  ??1ScriptCommandMessageEvent\@\@QEAA\@XZ
     */
    MCAPI ~ScriptCommandMessageEvent();

};