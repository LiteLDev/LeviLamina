/**
 * @file  ActorDefinitionAttribute.hpp
 *
 */
#pragma once
#define AUTO_GENERATED
#include "llapi/Global.h"

#define BEFORE_EXTRA

#undef BEFORE_EXTRA


struct ActorDefinitionAttribute {

#define AFTER_EXTRA

#undef AFTER_EXTRA
#ifndef DISABLE_CONSTRUCTOR_PREVENTION_ACTORDEFINITIONATTRIBUTE
public:
    struct ActorDefinitionAttribute& operator=(struct ActorDefinitionAttribute const &) = delete;
    ActorDefinitionAttribute(struct ActorDefinitionAttribute const &) = delete;
    ActorDefinitionAttribute() = delete;
#endif

public:
    /**
     * @symbol  ??1ActorDefinitionAttribute\@\@QEAA\@XZ
     */
    MCAPI ~ActorDefinitionAttribute();

};