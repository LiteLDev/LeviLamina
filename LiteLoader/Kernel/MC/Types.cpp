...
{
    float mr = r > 0.04045f ? pow(r / 1.055f + 0.055f, 2.4f)
                            : r / 12.92f;
    float mg = g > 0.04045f ? pow(g / 1.055f + 0.055f, 2.4f)
                            : g / 12.92f;
    float mb = b > 0.04045f ? pow(b / 1.055f + 0.055f, 2.4f)
                            : b / 12.92f;
    return Color(mr, mg, mb, a);
};
mce::Color Color::LinearTosRGB() const
{
    float mr = r > 0.0031308f ? 1.055f * pow(r, 1.0f / 2.4f) - 0.055f
                              : r * 12.92f;
    float mg = g > 0.0031308f ? 1.055f * pow(g, 1.0f / 2.4f) - 0.055f
                              : g * 12.92f;
    float mb = b > 0.0031308f ? 1.055f * pow(b, 1.0f / 2.4f) - 0.055f
                              : b * 12.92f;
    return Color(mr, mg, mb, a);
};
...
double mce::Color::deltaE00(mce::Color const& dst) const
{
    ...
    double T = 1 - 0.17 * cos((H_p - 30) * 0.0174532925199) + 0.24 * cos((2 * H_p) * 0.0174532925199) + 0.32 * cos((3 * H_p + 6) * 0.0174532925199) - 0.20 * cos((4* H_p - 63.0) * 0.0174532925199);
