#include "liteloader/api/db/impl/mysql/Stmt.h"
#include "liteloader/api/LoggerAPI.h"
#include "liteloader/api/db/impl/mysql/Session.h"
...
/**
 * @brief Judge if the field is uint type
 *
 * @param  field  Field to judge
 * @return bool   True if the field is uint
 */
...
Any ReceiverToAny(const Receiver& rec) {
    ...
    case MYSQL_TYPE_TINY:
        if (IsUnsigned(rec.field))
            return *reinterpret_cast<uchar*>(rec.buffer.get());
        return *reinterpret_cast<char*>(rec.buffer.get());
    case MYSQL_TYPE_SHORT:
        if (IsUnsigned(rec.field))
            return *reinterpret_cast<ushort*>(rec.buffer.get());
        return *reinterpret_cast<short*>(rec.buffer.get());
    case MYSQL_TYPE_INT24:
    case MYSQL_TYPE_LONG:
        if (IsUnsigned(rec.field))
            return *reinterpret_cast<uint*>(rec.buffer.get());
        return *reinterpret_cast<int*>(rec.buffer.get());
    case MYSQL_TYPE_BIT:
        return *reinterpret_cast<uint64*>(rec.buffer.get());
    case MYSQL_TYPE_LONGLONG:
        if (IsUnsigned(rec.field))
            return *reinterpret_cast<uint64*>(rec.buffer.get());
        return *reinterpret_cast<int64*>(rec.buffer.get());
    case MYSQL_TYPE_FLOAT:
...
: Stmt(parent, autoExecute), stmt(stmt) {
    ...
    if (totalParamsCount)
        params = std::make_shared<MYSQL_BIND[]>(totalParamsCount);
    else {
        if (autoExecute)
...
void MySQLStmt::bindResult() {
    ...
    for (auto i = 0U; i < cnt; i++) {
        ...
        data.buffer_length   = (ulong)len;            // Set the buffer length
        ...
        IF_ENDBG dbLogger.debug(
            "MySQLStmt::bindResult: Bind result {} (type {}) at index {}", field.name, (int)field.type, i
        );
    }
...
Stmt& MySQLStmt::bind(const Any& value, int index) {
    ...
    case Any::Type::Integer:
        ...
        param.buffer.reset(new char[sizeof(int64)]);
        *reinterpret_cast<int64*>(param.buffer.get()) = value.value.integer;
        params[index].buffer                          = param.buffer.get();
        params[index].buffer_length                   = sizeof(int64);
    ...
    case Any::Type::UInteger:
        params[index].buffer_type = MYSQL_TYPE_LONGLONG;
        param.buffer.reset(new char[sizeof(uint64)]);
        *reinterpret_cast<uint64*>(param.buffer.get()) = value.value.uinteger;
        params[index].buffer                           = param.buffer.get();
        params[index].buffer_length                    = sizeof(uint64);
        params[index].is_unsigned                      = true;
        params[index].is_null                          = nullptr;
        params[index].length                           = nullptr;
        break;
    ...
    case Any::Type::String: {
        ...
        params[index].is_null       = nullptr;
        params[index].length        = (ulong*)&param.length;
...
uint64 MySQLStmt::getAffectedRows() const { return mysql_stmt_affected_rows(stmt); }

uint64 MySQLStmt::getInsertId() const { return mysql_stmt_insert_id(stmt); }
