...
Stmt& SQLiteStmt::bind(const Any& value, int index) {
    ...
    case Any::Type::Integer:
        res = sqlite3_bind_int64(stmt, index, value.get<int64>());
        break;
    case Any::Type::UInteger: {
        uint64 val = value.get<uint64>();
        if (val > LLONG_MAX) {
            ...
            res = sqlite3_bind_int64(stmt, index, (int64)val);
        }
        break;
    }
...
Row SQLiteStmt::_Fetch() {
    ...
    for (int i = 0; i < (int)(resultHeader->size()); i++) {
        ...
        case SQLITE_BLOB: {
            ByteArray arr(
                reinterpret_cast<const uchar*>(sqlite3_column_blob(stmt, i)),
                reinterpret_cast<const uchar*>(sqlite3_column_blob(stmt, i)) + sqlite3_column_bytes(stmt, i)
            );
            IF_ENDBG {
...
uint64 SQLiteStmt::getAffectedRows() const { return affectedRowCount; }

uint64 SQLiteStmt::getInsertId() const { return insertRowId; }
