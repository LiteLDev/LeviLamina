...
void Header::setupSubscriptions(
    ...
    if (std::holds_alternative<std::shared_ptr<ObservableString>>(mText)) {
        auto obs = std::get<std::shared_ptr<ObservableString>>(mText);
        if (obs) {
            auto subId =
                obs->subscribe([updateString, prefix](std::string const& val) { updateString(prefix + "text", val); });
