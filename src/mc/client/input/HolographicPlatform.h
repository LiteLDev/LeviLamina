...
public:
    ...
    // NOLINTBEGIN
    MCAPI void $init(::std::shared_ptr<::Options>);

    MCAPI void $preInitUpdate();

    MCAPI void $update(::Bedrock::NotNullNonOwnerPtr<::IClientInstance> const&);

    MCAPI void $postRenderUpdate(::Bedrock::NotNullNonOwnerPtr<::IClientInstance> const&);

    MCAPI void $getSoundSystemOutputDevice(::SoundSystemGUID&);
    ...
    MCAPI void $onHMDFocusGained();

    MCAPI void $onHMDFocusLost();

    MCAPI void $onInitialResourcesLoaded();

    MCAPI ::std::shared_ptr<::Bedrock::Threading::IAsyncResult<void>> $waitForOutputMode(::VROutputMode);

    MCAPI bool $useVRInputMode() const;

    MCAPI int $getWorkerFramerate();

    MCAPI bool $allowVRFrameExperience();

    MCAPI bool $allowSplitScreen();

    MCAPI bool $supportsLoadingScreen();

    MCAPI bool $pauseIsDesired();

    MCAPI bool $allowThirdPersonView();

    MCAPI bool $useSinglePassStereo();

    MCAPI bool $drawCubemapInMono();

    MCAPI bool $isVRMode() const;

    MCAPI bool $isARMode() const;

    MCAPI bool $isRenderingIn2D() const;

    MCAPI bool $isSessionVisible() const;

    MCAPI bool $needsPostRenderUpdate() const;

    MCAPI bool $deviceHasPositionalTracking();

    MCAPI bool $deviceNeedsToBeRemovedForXblSignin();

    MCAPI bool $deviceNeedsToBeRemovedForSkinPicker();

    MCAPI bool $deviceNeedsToBeRemovedForFilePicker();

    MCAPI ::VRHUDPosition $getDefaultVRHUDPosition() const;

    MCAPI void $setUndockHandler(::std::function<void()>);

    MCAPI int $numRequiredControllers();

    MCAPI ::std::string $getPlatformRecalibrationMessage() const;

    MCAPI ::std::string $getPlatformRecalibrationMessageTTS(::GamePadRemappingLayout const&) const;

    MCAPI ::std::string $getPlatformAlignmentPrompt(::InputMode) const;

    MCAPI ::std::string $getPlatformAlignmentPromptTTS(::GamePadRemappingLayout const&, ::InputMode) const;

    MCAPI bool $forceControllerUsage() const;

    MCAPI bool $separateEyePasses() const;

    MCAPI void $createRenderGraph(::mce::RenderGraph&, ::IClientInstance&);

    MCAPI bool $isGazeDevicePresent();

    MCAPI bool $isGestureDevicePresent();

    MCAPI bool $useAlternateTouchInput();

    MCAPI void $resetBaseFrameOfReference(float, bool);

    MCAPI ::HolographicPlatform::LUID $getHoloDeviceAdapterLUID();

    MCAPI void $captureRenderDevice();

    MCAPI bool $isLocated();

    MCAPI bool $isPrimaryHandHeld();

    MCAPI bool $isSecondaryHandHeld();

    MCAPI void $setScreenLocation(::Vec3 const&, ::Vec3 const*, float);

    MCAPI void $setScreenLocation(::Matrix const&);

    MCAPI void $setHeadlockedUITransform(::Matrix const&, float const);

    MCAPI void $setScreenLocationOffset(::Vec3 const&);

    MCAPI void $resetScreenLocationOffset();

    MCAPI void $setHoloviewerLocation(::Vec3 const&, ::Vec3 const*, float, bool);

    MCAPI void $setHoloviewerLocation(::Matrix const&);

    MCAPI void $setRealityAnchorLocation(::Matrix const&);

    MCAPI float $getViewableScreenSize(float);

    MCAPI void $defaultLSRPlane(::HolographicPlatform::MCLocation);

    MCAPI void $updateLSRPlane(::HolographicPlatform::MCLocation, ::Vec3 const&);

    MCAPI void $getLastLSRPlanePosAndNormPS(::Vec3&, ::Vec3&);

    MCAPI float $getSRCastDistance();

    MCAPI bool $startEnvironmentScanning();

    MCAPI void $stopEnvironmentScanning();

    MCAPI void $resetEnvironmentScan();

    MCAPI void $renderSRData(float, float);

    MCAPI bool $hideCursorOnFocusLost(::IClientInstance&) const;

    MCAPI bool $getGazeCastResultPS(::Vec3&, ::Vec3&, ::Vec3&);

    MCAPI bool $getStereoEnabled();

    MCAPI void $setStereoEnabled(bool);

    MCAPI void $delayForNextFingerPress(int);

    MCAPI float $getPreferredUIAspectRatio();

    MCAPI float $getPreferredLivingRoomLevelAspectRation();

    MCAPI float $getVRFOV() const;

    MCAPI float $getDefaultIngameUIDistance();

    MCAPI bool $shouldCompressHUDWidth() const;

    MCAPI void $getUIScreenWidthHeightScale(uint&, uint&, float&);

    MCAPI void $getLivingRoomLevelTextureWidthHeight(uint&, uint&, float&);

    MCAPI void $getItemInHandOffset(::Vec3&);

    MCAPI ::std::string const $getLivingRoomHintText(::std::string const&) const;

    MCAPI ::UIRenderMode $getUIRenderMode();

    MCAPI ::UIRenderMode $getVRHUDRenderMode();

    MCAPI float $getHudAlphaBlendFactor() const;

    MCAPI bool $getVRLowFrequencyHUD();

    MCAPI char const* $getHandMaterial() const;

    MCAPI bool $wantsToStealMouse();

    MCAPI bool $drawHolographicBordersForUI();

    MCAPI float $getDefaultHalfHoloScreenWidth();

    MCAPI float $getDefaultHalfHoloHUDWidth();

    MCAPI float $getAmbientBoost(float);

    MCAPI bool $clampToMinimumLight();

    MCAPI void $setUIClipRect(::RectangleArea const&);

    MCAPI float $getUIClipGrowPixels();

    MCAPI bool $isFeatureEnabled(uint);

    MCAPI ::std::optional<::MFC::VrConfiguration> $getVrConfiguration();

    MCAPI void $frameUpdate(::FrameUpdateContext&);

    MCAPI ushort $getLivingRoomViewSetId() const;

    MCAPI ::mce::framebuilder::LivingRoomDescription const* $getLivingRoomDescription() const;

    MCAPI float $getMinimumLivingRoomFrameDistance();

    MCAPI bool $neverStealMouse();

    MCAPI bool $present(::ScreenContext&);

    MCAPI bool $isWaitingForPresent() const;

    MCAPI bool $getAllowMirrorPresent();

    MCAPI bool $permitRendering();

    MCAPI bool $permitAudio();

    MCAPI void $_prepPlatformSpecificTransforms();

    MCAPI bool $_shouldRenderIn2D() const;
    // NOLINTEND
};
