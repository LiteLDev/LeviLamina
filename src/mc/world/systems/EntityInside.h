...
public:
     ...
     */
    // MCAPI static createContextObjects(class StrictExecutionContext<struct Filter<class FlagComponent<struct
    // ActorMovementTickNeededFlag>, class FlagComponent<struct BoatFlag>, class FlagComponent<struct FreezeImmuneFlag>,
    // class FlagComponent<struct IsDeadFlag>, class FlagComponent<struct PlayerComponentFlag>, class
    // FlagComponent<struct SkipEntityInsideFlag>, class FlagComponent<struct InWaterFlag>>, struct Read<struct
    // AABBShapeComponent, struct AbilitiesComponent, struct ActorGameTypeComponent, struct DimensionTypeComponent>,
    // struct Write<struct BlockMovementSlowdownMultiplierComponent, struct FallDistanceComponent, struct
    // InsideBlockComponent, struct StateVectorComponent>, struct AddRemove<struct
    // BlockMovementSlowdownAppliedComponent, class FreezingComponent, struct IgnoresEntityInsideFlagComponent, struct
    // InsideBubbleColumnBlockComponent, struct InsideBlockWithPosAndBlockComponent<struct CactusBlockFlag>, struct
    // InsideBlockWithPosAndBlockComponent<struct EndPortalBlockFlag>, struct InsideGenericBlockComponent, struct
    // InsideBlockWithPosAndBlockComponent<struct HoneyBlockFlag>, struct InsideBlockWithPosAndBlockComponent<struct
    // PowderSnowBlockFlag>, struct InsideBlockWithPosAndBlockComponent<struct SweetBerryBushBlockFlag>, struct
    // InsideBlockWithPosComponent<struct WaterlilyBlockFlag>, class FlagComponent<struct WebBlockTag>>, struct
