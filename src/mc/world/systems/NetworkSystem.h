...
#ifndef DISABLE_CONSTRUCTOR_PREVENTION_NETWORKSYSTEM_DEPENDENCIES
    public:
...
public:
     ...
     */
    MCAPI void
    _createNetworkStatistics(enum class TrackerType, class std::function<bool(struct RakNet::RakNetStatistics&)>&&, class gsl::not_null<class Bedrock::NonOwnerPointer<class NetworkDebugManager>>&&);
    /**
     ...
     */
    MCAPI bool
    _sortAndPacketizeEvents(class NetworkConnection&, class std::chrono::time_point<struct std::chrono::steady_clock, class std::chrono::duration<__int64, struct std::ratio<1, 1000000000>>>);
