#pragma once

#include "mc/_HeaderOutputPredefine.h"

class GameRule {
public:
    // GameRule inner types declare
    // clang-format off
    class ValidationError;
    // clang-format on

    // GameRule inner types define
    enum class Type {};

    class ValidationError {

    public:
        // prevent constructor by default
        ValidationError& operator=(ValidationError const&) = delete;
        ValidationError(ValidationError const&)            = delete;
        ValidationError()                                  = delete;

    public:
        /**
         * @symbol ??1ValidationError\@GameRule\@\@QEAA\@XZ
         */
        MCAPI ~ValidationError(); // NOLINT
    };

public:
    // prevent constructor by default
    GameRule() = delete;

public:
    /**
     * @symbol ??0GameRule\@\@QEAA\@$$QEAV0\@\@Z
     */
    MCAPI GameRule(class GameRule&&); // NOLINT
    /**
     * @symbol ??0GameRule\@\@QEAA\@AEBV0\@\@Z
     */
    MCAPI GameRule(class GameRule const&); // NOLINT
    /**
     * @symbol ??0GameRule\@\@QEAA\@AEBV?$basic_string\@DU?$char_traits\@D\@std\@\@V?$allocator\@D\@2\@\@std\@\@_N\@Z
     */
    MCAPI GameRule(std::string const&, bool); // NOLINT
    /**
     * @symbol ?allowUseInCommand\@GameRule\@\@QEBA_NXZ
     */
    MCAPI bool allowUseInCommand() const; // NOLINT
    /**
     * @symbol ?canBeModifiedByPlayer\@GameRule\@\@QEBA_NXZ
     */
    MCAPI bool canBeModifiedByPlayer() const; // NOLINT
    /**
     * @symbol ?getBool\@GameRule\@\@QEBA_NXZ
     */
    MCAPI bool getBool() const; // NOLINT
    /**
     * @symbol ?getFloat\@GameRule\@\@QEBAMXZ
     */
    MCAPI float getFloat() const; // NOLINT
    /**
     * @symbol ?getInt\@GameRule\@\@QEBAHXZ
     */
    MCAPI int getInt() const; // NOLINT
    /**
     * @symbol ?getName\@GameRule\@\@QEBAAEBV?$basic_string\@DU?$char_traits\@D\@std\@\@V?$allocator\@D\@2\@\@std\@\@XZ
     */
    MCAPI std::string const& getName() const; // NOLINT
    /**
     * @symbol ?getType\@GameRule\@\@QEBA?AW4Type\@1\@XZ
     */
    MCAPI enum class GameRule::Type getType() const; // NOLINT
    /**
     * @symbol ?getValue\@GameRule\@\@QEBAAEBTValue\@1\@XZ
     */
    MCAPI union GameRule::Value const& getValue() const; // NOLINT
    /**
     * @symbol ??4GameRule\@\@QEAAAEAV0\@AEBV0\@\@Z
     */
    MCAPI class GameRule& operator=(class GameRule const&); // NOLINT
    /**
     * @symbol ??4GameRule\@\@QEAAAEAV0\@$$QEAV0\@\@Z
     */
    MCAPI class GameRule& operator=(class GameRule&&); // NOLINT
    /**
     * @symbol ?requiresCheats\@GameRule\@\@QEBA_NXZ
     */
    MCAPI bool requiresCheats() const; // NOLINT
    /**
     * @symbol ?resetType\@GameRule\@\@QEAAXW4Type\@1\@\@Z
     */
    MCAPI void resetType(enum class GameRule::Type); // NOLINT
    /**
     * @symbol ?setBool\@GameRule\@\@QEAA_N_NPEA_NPEAVValidationError\@1\@\@Z
     */
    MCAPI bool setBool(bool, bool*, class GameRule::ValidationError*); // NOLINT
    /**
     * @symbol ?setFloat\@GameRule\@\@QEAA_NMPEA_NPEAVValidationError\@1\@\@Z
     */
    MCAPI bool setFloat(float, bool*, class GameRule::ValidationError*); // NOLINT
    /**
     * @symbol ?setInt\@GameRule\@\@QEAA_NHPEA_NPEAVValidationError\@1\@\@Z
     */
    MCAPI bool setInt(int, bool*, class GameRule::ValidationError*); // NOLINT
    /**
     * @symbol ?setTagDataNotFoundCallback\@GameRule\@\@QEAAAEAV1\@V?$function\@$$A6AXAEAVGameRule\@\@\@Z\@std\@\@\@Z
     */
    MCAPI class GameRule& setTagDataNotFoundCallback(class std::function<void(class GameRule&)>); // NOLINT
    /**
     * @symbol
     * ?setValidateValueCallback\@GameRule\@\@QEAAAEAV1\@V?$function\@$$A6A_NAEBTValue\@GameRule\@\@PEAVValidationError\@2\@\@Z\@std\@\@\@Z
     */
    MCAPI class GameRule&
        setValidateValueCallback(class std::function<
                                 bool(union GameRule::Value const&, class GameRule::ValidationError*)>); // NOLINT
    /**
     * @symbol ??1GameRule\@\@QEAA\@XZ
     */
    MCAPI ~GameRule(); // NOLINT

    // protected:
    /**
     * @symbol ?_setDefaultValue\@GameRule\@\@IEAAAEAV1\@H\@Z
     */
    MCAPI class GameRule& _setDefaultValue(int); // NOLINT
    /**
     * @symbol ?_setDefaultValue\@GameRule\@\@IEAAAEAV1\@_N\@Z
     */
    MCAPI class GameRule& _setDefaultValue(bool); // NOLINT

    // private:
    /**
     * @symbol ?_set\@GameRule\@\@AEAA_NAEBTValue\@1\@PEA_NPEAVValidationError\@1\@\@Z
     */
    MCAPI bool _set(union GameRule::Value const&, bool*, class GameRule::ValidationError*); // NOLINT

protected:
private:
};
