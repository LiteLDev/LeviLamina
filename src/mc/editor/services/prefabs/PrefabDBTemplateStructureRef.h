...
public:
    ...
    MCNAPI ::mce::UUID const& getStructureTemplateDataId() const;

    MCNAPI ::WeakRef<::Editor::Prefabs::PrefabDBTemplateStructureRef> getWeakRef();

    MCNAPI void setInstanceMirror(::Mirror mirror);

    MCNAPI void setInstanceOffset(::Vec3 const& offset);

    MCNAPI void setInstanceRotation(::Rotation rotation);
    // NOLINTEND
};
