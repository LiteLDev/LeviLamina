 ...
 */
MCAPI void _liquidBlockFetch(class StrictEntityContext const&, struct StateVectorComponent&, struct UpdateWaterStateRequestComponent&, struct AABBShapeComponent const&, struct SubBBsComponent const&, class Optional<class FlagComponent<struct ItemActorFlag> const>, std::vector<struct LiquidBlockEntry>&, class IConstBlockSource const&); // NOLINT
/**
 * @symbol
 * ?_markForLiquidFlow\@LiquidPhysicsSystem\@\@YAXAEAUUpdateWaterStateRequestComponent\@\@AEBV?$Optional\@$$CBUAbilitiesComponent\@\@\@\@\@Z
 */
MCAPI void
_markForLiquidFlow(struct UpdateWaterStateRequestComponent&, class Optional<struct AbilitiesComponent const> const&); // NOLINT
/**
 * @symbol
