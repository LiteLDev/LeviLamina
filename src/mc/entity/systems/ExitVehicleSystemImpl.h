...
public:
    ...
    // StopRidingRequestFlag>>, struct Read<struct AbilitiesComponent, struct ActorTypeComponent, struct
    // FallDistanceComponent, struct ActorGameTypeComponent, struct MobBodyRotationComponent, struct
    // RenderRotationComponent, struct StandAnimationComponent, struct StateVectorComponent, struct
    // SynchedActorDataComponent, struct AABBShapeComponent, struct ActorRotationComponent, class Optional<struct
    // VanillaOffsetComponent>, class Optional<struct PassengerRenderingRidingOffsetComponent>, struct
    // DimensionTypeComponent, struct PassengerComponent, struct VehicleComponent, struct OffsetsComponent, class
    // FlagComponent<struct EjectedByActivatorRailFlag>>, struct Write<struct AABBShapeComponent, struct
    // ActorRotationComponent, struct StateVectorComponent>, struct AddRemove<struct ActorSetPositionRequestComponent>,
    // struct GlobalRead<struct ExternalDataComponent, struct LocalConstBlockSourceFactoryComponent>, struct
    // GlobalWrite<>, struct EntityFactoryT<>>&); NOLINTEND
};
