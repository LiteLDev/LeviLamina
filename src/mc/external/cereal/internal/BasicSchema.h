...
public:
    ...
    // vIndex: 12
    virtual void constraint(std::unique_ptr<class cereal::Constraint> constraint);

    // vIndex: 13
    virtual class cereal::Constraint const* constraint() const;

    MCAPI explicit BasicSchema(struct cereal::ReflectionCtx const& ctx);

    MCAPI struct cereal::ReflectionCtx const& ctx() const;

    MCAPI void customError(std::string error);

    MCAPI struct cereal::SchemaDescription description() const;

    MCAPI bool isProperlyInitialized(entt::meta_any const& any) const;
    ...
    MCAPI void
    save(struct cereal::SchemaWriter& value, entt::meta_any const& any, class cereal::SerializerContext& context) const;
