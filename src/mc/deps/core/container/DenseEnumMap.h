...
public:
    ...
    constexpr reference operator[](Key key) noexcept { return mContainer[static_cast<size_type>(key)]; }

    constexpr const_reference operator[](Key key) const noexcept { return mContainer[static_cast<size_type>(key)]; }

    constexpr reference at(Key key) { return mContainer.at(static_cast<size_type>(key)); }

    constexpr const_reference at(Key key) const { return mContainer.at(static_cast<size_type>(key)); }
    ...
    constexpr void fill(Value const& value) { mContainer.fill(value); }
};
