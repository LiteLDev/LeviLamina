#pragma once

#include "mc/_HeaderOutputPredefine.h"

// auto generated inclusion list
#include "mc/deps/shared_types/legacy/BlockDescriptor.h"

// auto generated forward declare list
// clang-format off
namespace cereal { struct ReflectionCtx; }
// clang-format on

namespace SharedTypes::v1_26_50 {

struct PreferredPathComponentDefinition {
public:
    // PreferredPathComponentDefinition inner types declare
    // clang-format off
    struct BlockSet;
    // clang-format on

    // PreferredPathComponentDefinition inner types define
    struct BlockSet {
    public:
        // member variables
        // NOLINTBEGIN
        ::ll::TypedStorage<4, 8, ::std::optional<float>>                                 mCost;
        ::ll::TypedStorage<8, 24, ::std::vector<::SharedTypes::Legacy::BlockDescriptor>> mBlockDescriptors;
        // NOLINTEND
    };

public:
    // member variables
    // NOLINTBEGIN
    ::ll::TypedStorage<4, 4, float> mDefaultBlockCost;
    ::ll::TypedStorage<4, 4, float> mJumpCost;
    ::ll::TypedStorage<4, 4, int>   mMaxFallBlocks;
    ::ll::TypedStorage<8, 24, ::std::vector<::SharedTypes::v1_26_50::PreferredPathComponentDefinition::BlockSet>>
        mPreferredPathGroup;
    // NOLINTEND

public:
    // static functions
    // NOLINTBEGIN
    MCAPI static void bindType(::cereal::ReflectionCtx& ctx);
    // NOLINTEND

public:
    // static variables
    // NOLINTBEGIN
    MCAPI static ::std::string_view const& NAME();
    // NOLINTEND
};

} // namespace SharedTypes::v1_26_50
