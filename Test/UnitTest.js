...
const ll_test = {
    run: () => {
        ll.registerPlugin(
            "UnitTest",
            "LiteLoader Script Engine unit test",
            [0, 0, 1],
            { Author: "LiteLDev" }
        );
...
const http_server_test = {
    ...
    run: () => {
        ...
        this.server
            .onGet("/hello(.+)", (req, resp) => {
                logger.info(
                    "http_server_test: run: onGet: Received a request from ",
                    req.remoteAddr,
                    ":",
                    req.remotePort,
                    " for ",
                ...
                resp.write("<html><body>")
                    ...
                    .write(
                        "<p>Path Regex Matches: ",
                        JSON.stringify(req.matches),
                        "</p>"
                    )
                    .write("</body></html>");
                resp.status = 200;
            ...
            .onPreRouting((req, resp) => {
                logger.info(
                    "http_server_test: run: onPreRouting: resp.body.length: ",
                    resp.body.length
                );
            ...
            .onPostRouting((_req, resp) => {
                logger.info(
                    "http_server_test: run: onPostRouting: resp.body.length: ",
                    resp.body.length
                );
            })
            .onError((_req, resp) => {
        ...
        logger.info(
            "http_server_test: run: Server listening on " +
                this.listen_address +
                ":" +
                this.listen_port
        );
    ...
    command: {
        ...
        register: (cmd) => {
            ...
            cmd.mandatory(
                "http_server_action",
                ParamType.Enum,
                "http_server_action_stop"
            );
            cmd.overload(["cmd", "http_server_action_stop"]);
        },
        handler: (_cmd, _ori, _outp, res) => {
...
///////////////////////////////////// CLASSES //////////////////////////////////////
const CLASSES = [ll_test, file_test, http_server_test, json_config_file_test];
