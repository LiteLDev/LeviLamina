...
public:
    // member functions
    // NOLINTBEGIN
    MCAPI void _appendLogEntryMetadata(::std::string& logEntry, ::std::string timestamp, ::LogAreaID area, uint priority, ::std::string functionName, int lineNumber, int _messageId);

    MCAPI ::std::string _getCurrentTimestamp();

    MCAPI void _logToFile(::std::string const& debugEvent);

    MCAPI void _log_va(::LogAreaID _area, uint _priority, char const* _function, int line, int _messageId, char const* pszFormat, char* argptr);

    MCAPI ::std::string _makeLogString(::std::string timestamp, ::LogAreaID area, uint priority, ::std::string functionName, int lineNumber, int messageId, ::std::string const& logMessage);

    MCAPI void _openLogFile();

    MCAPI void closeLog();

    MCAPI void createLog(::Core::Path const& _logFilePath, ::std::string const& _logFileName, ::std::string const& _debugLogTimestamp, bool createLogFile, ::LogSettingsUpdater* _settings, double _logCycleInS);
