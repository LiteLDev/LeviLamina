...
public:
    // static functions
    // NOLINTBEGIN
    MCNAPI static int _parseTimeZone(char const* str, uint64 len, ::DateManager::TimeZoneType* type, int* hours, int* minutes);
    ...
    MCNAPI static bool toDateTime(::std::string const& strTime, ::tm* result, ::DateManager::TimeZoneType* resultType, int* resultTimeZoneMinutes);

    MCNAPI static int64 toEpochTime(::tm const* dateTime, ::DateManager::TimeZoneType inputType);

    MCNAPI static ::std::string toString(::tm const& time, ::std::string const& format, ::std::optional<::std::locale> const& locale);
