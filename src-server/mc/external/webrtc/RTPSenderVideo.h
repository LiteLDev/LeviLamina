...
public:
    ...
    // vIndex: 0
    virtual bool SendVideo(int payload_type, ::std::optional<::webrtc::VideoCodecType> codec_type, uint rtp_timestamp, ::webrtc::Timestamp capture_time, ::rtc::ArrayView<uchar const> payload, uint64 encoder_output_size, ::webrtc::RTPVideoHeader video_header, ::webrtc::TimeDelta expected_retransmission_time, ::std::vector<uint> csrcs) /*override*/;

    // vIndex: 1
    virtual void SetVideoStructureAfterTransformation(::webrtc::FrameDependencyStructure const* video_structure) /*override*/;
...
public:
    // member functions
    // NOLINTBEGIN
    MCNAPI void AddRtpHeaderExtensions(::webrtc::RTPVideoHeader const& video_header, bool first_packet, bool last_packet, ::webrtc::RtpPacketToSend* packet) const;

    MCNAPI bool AllowRetransmission(uchar temporal_id, int retransmission_settings, ::webrtc::TimeDelta expected_retransmission_time);

    MCNAPI uint64 FecPacketOverhead() const;

    MCNAPI void LogAndSendToNetwork(::std::vector<::std::unique_ptr<::webrtc::RtpPacketToSend>> packets, uint64 encoder_output_size);
    ...
    MCNAPI bool SendEncodedImage(int payload_type, ::std::optional<::webrtc::VideoCodecType> codec_type, uint rtp_timestamp, ::webrtc::EncodedImage const& encoded_image, ::webrtc::RTPVideoHeader video_header, ::webrtc::TimeDelta expected_retransmission_time);
...
public:
    ...
    // NOLINTBEGIN
    MCNAPI bool $SendVideo(int payload_type, ::std::optional<::webrtc::VideoCodecType> codec_type, uint rtp_timestamp, ::webrtc::Timestamp capture_time, ::rtc::ArrayView<uchar const> payload, uint64 encoder_output_size, ::webrtc::RTPVideoHeader video_header, ::webrtc::TimeDelta expected_retransmission_time, ::std::vector<uint> csrcs);
