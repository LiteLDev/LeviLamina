...
public:
    ...
    // vIndex: 5
    virtual bool isFeatureChunk(::BiomeSource const&, ::Random&, ::ChunkPos const&, uint, ::IPreliminarySurfaceProvider const&, ::Dimension const&) = 0;

    // vIndex: 6
    virtual ::std::unique_ptr<::StructureStart> createStructureStart(::Dimension&, ::BiomeSource const&, ::Random&, ::ChunkPos const&, ::IPreliminarySurfaceProvider const&) = 0;
...
public:
    // static functions
    // NOLINTBEGIN
    MCAPI static bool findNearestFeaturePositionBySpacing(::Dimension& dimension, ::IPreliminarySurfaceProvider const& preliminarySurfaceLevel, ::StructureFeature& feature, ::std::optional<::HashedString> const& biomeTag, ::BiomeSource const& biomeSource, ::BlockPos const& origin, ::BlockPos& result, int featureSpacing, int minFeatureSeparation, int randomSalt, bool tiltedSpacing, int maxSearchRadius, bool mustBeInNewChunks);

    MCAPI static ::ChunkPos getChunkPosInSpace(::ChunkPos const& cp, ::Random& random, uint levelSeed, int spacing, int separation, int salt, bool tiltedSpacing);
    // NOLINTEND
